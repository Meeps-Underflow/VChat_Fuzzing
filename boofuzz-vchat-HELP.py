...
def main():
    ...
    csv_logger = FuzzLoggerCsv(open("csv/help_fuzz_log.csv", "w", newline=""))  # Logs fuzzing progress to a CSV file
    ...
    s_initialize("HELP")  # Initialize a fuzzing block named "GMON"
    s_string('HELP', fuzzable=False, name='HELP-Command')  # Send the command "GMON" (not fuzzable)
    s_delim(' ', fuzzable=False, name='HELP-Space')  # Send a space after "GMON" (not fuzzable)
    s_string('A', name='HELP-STRING')  # Fuzz this part by sending different "A" variations
    s_static('\r\n', name='HELP-CRLF')  # Append a carriage return + newline to mimic real commands

    # session.pre_send = receive_banner  # Assign `receive_banner` as a pre-send callback to read any initial response

    session.connect(s_get("HELP"))  # Connect the fuzzing session to the "GMON" structure defined above
