...
def main():
    ...
    csv_logger = FuzzLoggerCsv(open("csv/trun_fuzz_log.csv", "w", newline=""))
    ...
    session = Session(
        ...
        target=Target(connection=TCPSocketConnection(host, int(port)), monitors=[mon]),  # Set up a TCP connection to the target with new monitor
