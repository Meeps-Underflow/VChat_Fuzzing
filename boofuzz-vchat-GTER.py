...
def main():
    ...
    csv_logger = FuzzLoggerCsv(open("csv/gter_fuzz_log.csv", "w", newline=""))  # Logs fuzzing progress to a CSV file
    ...
    # Define the fuzzing structure for the "GTER" command
    s_initialize("GTER")  # Initialize a fuzzing block named "GTER"
    s_string('GTER', fuzzable=False, name='GTER-Command')  # Send the command "GTER" (not fuzzable)
    s_delim(' ', fuzzable=False, name='GTER-Space')  # Send a space after "GTER" (not fuzzable)
    s_string('A', name='GTER-STRING')  # Fuzz this part by sending different "A" variations
    s_static('\r\n', name='GTER-CRLF')  # Append a carriage return + newline to mimic real commands

    # session.pre_send = receive_banner  # Assign `receive_banner` as a pre-send callback to read any initial response

    session.connect(s_get("GTER"))  # Connect the fuzzing session to the "GTER" structure defined above
